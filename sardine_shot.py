...
def _load_config():
    ...
    return save_folder, interval, retain_days


def _save_config(save_folder, interval, retain_days):
    config = configparser.ConfigParser()
    config_path = os.path.join(_get_base_dir(), 'config.ini')
    config.read(config_path)
...
SAVE_FOLDER, INTERVAL, RETAIN_DAYS = _load_config()
...
# --- Settings window ---
def _show_settings():
    global SAVE_FOLDER, INTERVAL, RETAIN_DAYS
    ...
    tk.Label(win, textvariable=error_var, fg=CLR_ERR, font=FONT,
        ...
        row=6, column=0, columnspan=3, padx=12, pady=(4, 2))
    ...
    def on_save():
        ...
        INTERVAL     = interval
        RETAIN_DAYS  = retain
        _save_config(folder, interval, retain)
        _settings_event.set()   # wake the screenshot loop so new settings apply immediately
        win.destroy()

    # ── Buttons ───────────────────────────────────────────────────────────────
    btn_frame = tk.Frame(win)
    btn_frame.grid(row=7, column=0, columnspan=3, pady=(6, 14))
